...
def show_progress(operation):
    if print_distance == False and print_frequency == False and print_papers == False and print_similarity_score == False:
        print('hi')
        global counter
        counter = counter + 1
...
callbacks = [
    ...
    # Cost Callbacks
    lambda paper: get_frequency(paper, k_value, DBLP_hashtable, 'DBLP', frequencies) if DBLP_parsing == True and generate_dblp_hashtable == True and calculate_frequency_dblp == True else None,
    lambda paper: get_frequency(paper, k_value, MAG_hashtable, 'MAG', frequencies) if MAG_parsing == True and generate_mag_hashtable == True and calculate_frequency_mag == True else None,
    lambda paper: get_frequency(paper, k_value, DBLP_hashtable, 'MAG', frequencies) if MAG_parsing == True and generate_mag_hashtable == True and DBLP_parsing == True and generate_dblp_hashtable == True and calculate_frequency_mag_dblp == True else None,
    lambda paper: get_frequency(paper, k_value, MAG_hashtable, 'DBLP', frequencies) if MAG_parsing == True and generate_mag_hashtable == True and DBLP_parsing == True and generate_dblp_hashtable == True and calculate_frequency_dblp_mag == True else None,
...
def get_frequency(paper, k, hashtable, query_source, output):
    if paper.file_source == query_source and paper.title:
        ...
        if frequency > 0 and print_frequency:
            print(F'Matching Title: "{title}"\nSource File: {query_source}, Shingles Frequency: {frequency} in {len(output)} papers.\n')
